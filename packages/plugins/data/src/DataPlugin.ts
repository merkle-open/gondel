...
export function hookDataDecoratorIntoCore() {
	...
	addGondelPluginEventListener(
		'Data',
		'start',
		function (gondelComponents: GondelComponent<HTMLElement>[], _: any | undefined, next: (result: any) => any) {
			gondelComponents.forEach((gondelComponent: GondelComponent) => {
				...
				componentDataBindings.forEach(([propertyKey, attributeKey, serializer]: DataBindingConfig) => {
					...
					if (initialValue) {
						(gondelComponent as any)[propertyKey] = (gondelComponent as any)[propertyKey] || initialValue;
